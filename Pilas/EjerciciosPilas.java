...
public class EjerciciosPilas {
    ...
    // Ejercicio escribir un string al reves
    static String escribirAlReves(String original) {
        PilasEstaticas volteador = new PilasEstaticas(original.length());
        String volteado = "";
        for (int i = 0; i < original.length(); i++) {
            volteador.insertar(original.charAt(i));
        }
        for (int i = 0; i < original.length(); i++) {
            volteado += volteador.pop();
        }
        return volteado;
    }
    ...
    public static void main(String[] arg) {
        ...
        // }
        // System.out.println(escribirAlReves("Esto debe estar al reves"));
